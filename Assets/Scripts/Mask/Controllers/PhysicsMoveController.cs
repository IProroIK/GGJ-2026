...
public class PhysicsMoveController : MonoBehaviour
{
    ...
    {
        if (_currentDraggedObject != null && _maskManager.CurrentMask == Enums.MaskType.Mover)
        {
    ...
    private void FixedUpdate()
    {
        if (_currentDraggedObject != null && _maskManager.CurrentMask == Enums.MaskType.Mover)
        {
            ...
            Vector3 dampingForce = -_currentDraggedObject.linearVelocity;
            dampingForce.y = 0f;
            _currentDraggedObject.AddForce(dampingForce * _dragDamping, ForceMode.Force);
        }
    }
